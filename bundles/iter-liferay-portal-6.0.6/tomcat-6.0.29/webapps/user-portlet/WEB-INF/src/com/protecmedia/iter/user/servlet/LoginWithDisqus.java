...
public class LoginWithDisqus extends HttpServlet implements Servlet
{
	private static final long serialVersionUID = 1L;
	
	private static Log _log = LogFactoryUtil.getLog(LoginWithDisqus.class);
				...
				if(groupId > 0)
				{
					...
					if(Validator.isNull(code))
					{
						...
						if(Validator.isNull(error))
						{
							...
							response.sendRedirect(DisqusUtil.getCodeURL(request, scope, groupId));
						}
						else
						{
					...
					else
					{
						...
						IterRegisterServiceUtil.preRegisterUser(request, response, 
								SocialNetworkUtil.SOCIAL_NAME_DISQUS, groupId, 
								DisqusUtil.getDetailsByCode(request, code, groupId));
