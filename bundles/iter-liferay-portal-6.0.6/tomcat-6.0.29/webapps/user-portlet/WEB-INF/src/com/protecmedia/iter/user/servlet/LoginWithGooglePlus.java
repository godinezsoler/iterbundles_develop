...
public class LoginWithGooglePlus extends HttpServlet implements Servlet
{
	private static final long serialVersionUID = 1L;
	
	private static Log _log = LogFactoryUtil.getLog(LoginWithGooglePlus.class);
				...
				if(groupId > 0)
				{
					...
					if(Validator.isNull(code))
					{
						...
						if(Validator.isNull(error))
						{
							...
							response.sendRedirect(GooglePlusUtil.getCodeURL(request, scope, groupId));
						}
						else
						{
					...
					else
					{
						...
						IterRegisterServiceUtil.preRegisterUser(request, response, 
								SocialNetworkUtil.SOCIAL_NAME_GOOGLEPLUS, groupId, 
								GooglePlusUtil.getDetailsByCode(request, code, groupId));
