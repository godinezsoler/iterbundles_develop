...
{
	portletTitle = UserTools.getLoginTitle(themeDisplay.getScopeGroupId(), request);	
}
else
{
...
<c:choose>
	<c:when test="<%= themeDisplay.isWidget() %>">
	...
	<c:when test="<%= themeDisplay.isStatePopUp() %>">
		<div class="portlet-body <%= HtmlUtil.getRemovableClass()%> ">
		...
		<c:choose>
			<c:when test="<%= portletDecorate %>">
				<liferay-theme:wrap-portlet page="portlet.jsp">
					<div class="<%= portletDisplay.isStateMin() ? "aui-helper-hidden" : "" %> <%= HtmlUtil.getRemovableClass()%> portlet-content-container" <%= containerStyles %>>
						<%@ include file="/html/common/themes/portlet_content_wrapper.jspf" %>
					</div>
