	...
	function(A) {
		...
		var Layout = {
			...
			updatePortletDropZones: function(portletBoundary) {
				var options = Layout.options;
				var portletDropNodes = portletBoundary.all(options.portletDropNodes);
			...
			_onPortletClose: function(event) {
				...
				if (column) {
					$(portlet._node).removeClass("portlet-boundary");
					$(portlet._node).find(".portlet-boundary").removeClass("portlet-boundary");
