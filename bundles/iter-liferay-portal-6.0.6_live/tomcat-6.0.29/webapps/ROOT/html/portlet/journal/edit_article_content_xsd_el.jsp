...
<!-- ITER: Se modifica el comportamiento del JSP para los campos con nombre Milenium (Estilo Lista) -->
<c:choose>
	<c:when test='<%= elName.equals("Milenium") %>'>
   		<li style="border-top:0" class="structure-field <%= css.trim() %>" <%= parentStructureData %> dataInstanceId='<%= elInstanceId %>' dataName='<%= elName %>' dataRepeatable='<%= elRepeatable %>' dataType='<%= elType %>' dataIndexType='<%= elIndexType %>' <%= metaData %>>
	</c:when>
	<c:otherwise>
	...
	<span class="folder">
		<div class="field-container">
			...
			<!-- ITER: Se modifica el comportamiento del JSP para los campos con nombre Milenium (Etiqueta) -->
			<c:choose>
				<c:when test='<%= elName.equals("Milenium") %>'>
					<label class="journal-article-field-label" style="display:none"><span><%= elLabel %></span></label>
			    </c:when>
			    <c:otherwise>
			      	<label class="journal-article-field-label"><span><%= elLabel %></span></label>
			    </c:otherwise>
			</c:choose>
			...
			<div class="journal-article-component-container">
				<c:if test='<%= elType.equals("text") %>'>
					...
					<!-- ITER: Se modifica el comportamiento del JSP para los campos con nombre Milenium (Estilo campo) -->
					<c:choose>
					      <c:when test='<%= elName.equals("Milenium") %>'>
					      	<aui:input  style="border:0; background:none" prefix="Milenium:" disabled="true" cssClass="lfr-input-text-container" label="" name="text" size="55" type="text" value="<%= elContent %>" />
					      </c:when>
					      <c:otherwise>
			...
			<!-- ITER: Se modifica el comportamiento del JSP para los campos con nombre Milenium (Localizacion) -->
			<c:if test='<%= !elName.equals("Milenium") %>'>
